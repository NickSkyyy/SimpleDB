...
public class BTreeReverseScanTest extends SimpleDbTestBase {
    ...
    @Test public void testRewindPredicates() throws IOException, DbException, TransactionAbortedException {
        ...
        scan.open();
        int maxLen = tuplesFiltered.size() - 1;
        for (int i = 0; i < tuplesFiltered.size(); ++i) {
            assertTrue(scan.hasNext());
            Tuple t = scan.next();
            //System.out.println(tuplesFiltered.get(i) + " " + SystemTestUtil.tupleToList(t));
            assertEquals(tuplesFiltered.get(maxLen - i), SystemTestUtil.tupleToList(t));
        }

        scan.rewind();
        maxLen = tuplesFiltered.size() - 1;
        for (int i = 0; i < tuplesFiltered.size(); ++i) {
            assertTrue(scan.hasNext());
            Tuple t = scan.next();
            assertEquals(tuplesFiltered.get(maxLen - i), SystemTestUtil.tupleToList(t));
...
/*
    public static junit.framework.Test suite() {
        return new junit.framework.JUnit4TestAdapter(BTreeReverseScan.class);
    }
